...
static void repopulateArrary(double* row, GridFragment *frag, int rowNr) {
    for (int col = 0; col < frag->gridDimension; col++) {
        row[col] = GP(frag, rowNr, col);
    }
}
...
// static void getWhoReceivesFromMe(int numProcesses, int myRank) {
...
static std::tuple<int, double> performAlgorithm(
  int myRank, int numProcesses, GridFragment *frag, double omega, double epsilon) {
    ...
    double maxDiff = 0;
    int numIterations = 0;
    int fragSize = frag->gridDimension;
